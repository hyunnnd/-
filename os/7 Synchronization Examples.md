...
- 철학자 4번만 chopstick을 **다른 순서**로 집게 함
- 이렇게 하면 **모든 철학자가 각자 한 개의 젓가락만 들고** 다른 하나를 기다리는 **대기 사이클**이 발생하지 않음 → **교착 상태 방지**
...
- 또 다른 가능한 해결책:    
    - **최대 4명의 철학자만** 테이블에 동시에 앉을 수 있도록 제한함
    - **두 젓가락이 모두 사용 가능할 때만** 철학자가 집게 함
    - **홀수 번호 철학자**는 **왼쪽 → 오른쪽** 순으로,
    - **짝수 번호 철학자**는 **오른쪽 → 왼쪽** 순으로 젓가락을 집음

- 위의 해결책들은 **데드락**을 제거할 수는 있지만    
    - **굶주림(starvation)** 가능성은 완전히 제거하지 못함