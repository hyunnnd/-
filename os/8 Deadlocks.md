...
- **시스템이 교착 상태에 들어가는 것을 허용**하고, 이후에 이를 **탐지(detection)**하는 접근 방식임
- 탐지를 위한 **알고리즘이 필요**함

#### 탐지 알고리즘 종류

- **자원 유형이 단일 인스턴스일 경우**  
    → Resource Allocation Graph를 사용하여 **Cycle(순환)**이 있는지 검사  
    → Cycle 존재 시 교착 상태 발생
- **자원 유형이 다중 인스턴스일 경우**  
    → 복잡한 알고리즘 사용 (이 강의에서는 생략, 추가 슬라이드에서 다룸)

#### Recovery Scheme (복구 방법)

- 교착 상태가 감지되면 다음과 같은 방식으로 복구 가능함:
    - **Terminate**: 교착 상태에 있는 프로세스를 종료함
    - **Resource Preemption**: 자원을 강제로 회수하고 다른 프로세스에 할당함
