...
> 교착 상태가 발생할 **필요 조건 중 하나 이상을 사전에 차단**함으로써, 교착 상태를 원천적으로 막는 방법입니다.
◼ 교착 상태가 발생하는 **필요 조건 중 적어도 하나가 성립하지 않도록 보장하는** 일련의 방법들입니다.  
◼ 자원 요청 방식에 **제약을 두는** 방식을 사용합니다.

## 🧮 Deadlock Avoidance (교착 상태 회피)

> 교착 상태가 발생할 수 있는 **불안전한 상태로 시스템이 진입하지 않도록**, **운영체제가 결정적으로 회피**하는 방법입니다.
◼ **교착 상태가 절대로 발생하지 않는 안전한 상태(safe state)**를 시스템이 유지하도록 하는 방법입니다.  
◼ 이를 위해 **현재 사용 가능한 자원**이나 **각 스레드에 할당된 자원**에 대한 정보가 필요합니다.  
◼ 또한 각 스레드가 **앞으로 요청하거나 해제할 자원에 대한 추가 정보**도 필요합니다.
