...
- **BGP 광고 경로:** 접두사 + 속성
    ...
    - **두 가지 중요한 속성:**
        ...
        - **NEXT-HOP:** 다음-hop AS로 향하는 특정 내부 AS 라우터를 표시

- **정책 기반 라우팅:**    
    - 경로 광고를 수신하는 게이트웨이는 **수입 정책**을 사용하여 경로를 수락하거나 거부함 (예: AS Y를 거쳐서 라우팅하지 않음)
    - AS 정책은 다른 AS들에게 경로를 **광고할지 여부**를 결정함


## **BGP 경로 광고**

- **AS2 라우터 2c**는 **AS3 라우터 3a**로부터 **AS3,X** 경로 광고를 수신 (eBGP를 통해)
- AS2의 정책에 따라, **AS2 라우터 2c**는 **AS3,X** 경로를 수락하고 이를 모든 AS2 라우터에 전파 (iBGP를 통해)
- AS2의 정책에 따라, **AS2 라우터 2a**는 **AS2, AS3, X** 경로를 **AS1 라우터 1c**로 광고 (eBGP를 통해)


## **BGP 경로 광고 (추가)**

게이트웨이 라우터는 목적지로 가는 여러 경로를 배울 수 있음:

- **AS1 게이트웨이 라우터 1c**는 **2a**로부터 경로 **AS2, AS3, X**를 배움
- **AS1 게이트웨이 라우터 1c**는 **3a**로부터 경로 **AS3, X**를 배움

**정책**에 따라, **AS1 게이트웨이 라우터 1c**는 **AS3, X** 경로를 선택하고, 이 경로를 **AS1 내**에서 iBGP를 통해 광고함.

- **1a, 1b, 1d**는 **iBGP**를 통해 **1c**로부터 "X로 가는 경로는 1c를 거친다"는 정보를 학습
- **1d**에서 **OSPF intra-domain 라우팅**을 통해, **1c**로 가려면 **인터페이스 1**을 사용해야 함
- **1d**에서 **X**로 가려면 **인터페이스 1**을 사용해야 함

**로컬 링크 인터페이스:**

- **1a, 1d**의 로컬 링크 인터페이스에서 목적지와 해당 인터페이스 정보를 표시합니다.

## **핫포테이토 라우팅**

- **2d**는 **iBGP**를 통해 **X**로의 경로를 **2a** 또는 **2c**를 통해 배울 수 있음을 알게 됨.
- **핫포테이토 라우팅:** 가장 적은 **intra-domain** 비용을 가진 로컬 게이트웨이를 선택 (예: **2d**는 **2a**를 선택, 비록 더 많은 AS 홉을 거쳐 **X**에 도달하더라도): **inter-domain 비용**에 대해 걱정하지 않음!

**OSPF 링크 가중치**가 경로 선택에 영향을 미치며, **2d**는 가장 작은 **intra-domain** 비용을 기준으로 경로를 선택합니다.


## **BGP: 광고를 통한 정책 달성**

- **ISP**는 고객 네트워크 간 트래픽만 라우팅하고, 다른 ISP 간의 전송 트래픽을 처리하지 않으려 합니다. (일반적인 "실제 세계" 정책)
- **A**는 경로 **Aw**를 **B**와 **C**에 광고합니다.
- **B**는 경로 **BAw**를 **C**에 광고하지 않기로 선택합니다!
    - **B**는 **C**에게 **CBAw**를 라우팅하는 데 대한 "수익"을 얻지 않습니다. 왜냐하면 **C**, **A**, **w**는 **B**의 고객이 아니기 때문입니다.
    - **C**는 **CBAw** 경로에 대해 알지 못합니다.
    - **C**는 **B**를 사용하지 않고 **CAw** 경로를 통해 **w**로 라우팅합니다.


## **BGP: 광고를 통한 정책 달성 (추가)**

- **ISP**는 고객 네트워크 간의 트래픽만 라우팅하고, 다른 ISP 간의 전송 트래픽을 처리하지 않으려 합니다. (일반적인 "실제 세계" 정책)
- **A**, **B**, **C**는 **제공자 네트워크**
- **x**, **w**, **y**는 **고객 네트워크** (제공자 네트워크의 고객)
- **x**는 **이중 연결**된 네트워크: 두 개의 네트워크에 연결됨
- **정책 강제:** **x**는 **B**에서 **C**로의 라우팅을 원하지 않음, 따라서 **x**는 **B**에 **C**로 가는 경로를 광고하지 않음


## **BGP 경로 선택**

- 라우터는 목적지 AS로 가는 경로를 여러 개 배울 수 있으며, 경로는 다음 기준에 따라 선택됩니다:
    1. **로컬 선호도 값** 속성: 정책 결정
    2. **가장 짧은 AS-PATH**
    3. **가장 가까운 NEXT-HOP 라우터**: 핫포테이토 라우팅
    4. **추가적인 기준**


## **BGP 메시지**

- **BGP 메시지**는 피어 간에 TCP 연결을 통해 교환됩니다.

**BGP 메시지 종류:**

- **OPEN:** 원격 BGP 피어로 TCP 연결을 열고, 보내는 BGP 피어를 인증합니다.
- **UPDATE:** 새로운 경로를 광고하거나 (기존 경로를 철회)
- **KEEPALIVE:** 업데이트가 없을 경우 연결을 유지하며, OPEN 요청에 대한 확인도 합니다.
- **NOTIFICATION:** 이전 메시지에서 발생한 오류를 보고하며, 연결을 종료하는 데 사용됩니다.


## **왜 다른 Intra-AS, Inter-AS 라우팅?**

- **정책 (Policy):**
    - **Inter-AS:** 관리자는 트래픽이 자신의 네트워크를 통해 어떻게 라우팅되는지, 누가 라우팅하는지 제어하고자 함.
    - **Intra-AS:** 단일 관리자가 관리하므로 정책은 덜 중요함.

- **규모 (Scale):**    
    - 계층적 라우팅은 테이블 크기를 절약하고, 업데이트 트래픽을 줄여줌.

- **성능 (Performance):**    
    - **Intra-AS:** 성능에 집중할 수 있음.
    - **Inter-AS:** 정책이 성능을 지배함.


## **소프트웨어 정의 네트워킹 (SDN)**

- **인터넷 네트워크 계층**: 역사적으로 분산된, 라우터별 제어 접근 방식으로 구현됨:
    - **모놀리식 라우터**는 스위칭 하드웨어를 포함하고, 인터넷 표준 프로토콜(IP, RIP, IS-IS, OSPF, BGP)을 독점적인 라우터 운영체제(예: Cisco IOS)에서 실행
    - 다양한 네트워크 계층 기능을 위한 **"미들박스"**들: 방화벽, 로드 밸런서, NAT 박스 등
- **~2005:** 네트워크 제어 평면에 대한 재고가 필요하다는 관심이 다시 일어남


## **소프트웨어 정의 네트워킹 (SDN)**

**왜 논리적으로 중앙 집중화된 제어 평면이 필요한가?**

- **더 쉬운 네트워크 관리**: 라우터 구성 오류를 피하고, 트래픽 흐름의 유연성을 높일 수 있음
- **테이블 기반 포워딩** (OpenFlow API 참고)은 라우터를 "프로그래밍" 할 수 있게 해줌
    - **중앙 집중화된 "프로그래밍"**은 더 쉬움: 테이블을 중앙에서 계산하고 배포
    - **분산된 "프로그래밍"**은 더 어려움: 각 라우터에 구현된 분산 알고리즘(프로토콜)을 기반으로 테이블을 계산해야 함
- **개방된 (비독점적인) 제어 평면 구현**
    - 혁신을 촉진: "천 개의 꽃이 피게 하라" (다양한 혁신을 가능하게 함)



## **트래픽 엔지니어링: 전통적인 라우팅에서는 어려움**

**Q:** 네트워크 운영자가 **u**에서 **z**로 가는 트래픽이 **uvwz** 경로를 따라 흐르기를 원한다고 할 때, **uxyz** 대신 어떻게 해야 하나요?

**A:** 트래픽 라우팅 알고리즘이 해당 경로를 계산하도록 하려면 링크 가중치를 재정의해야 합니다 (또는 새로운 라우팅 알고리즘이 필요할 수 있음)!

**링크 가중치**는 제어할 수 있는 "조정 장치"에 불과합니다: **많은 제어가 불가능합니다!**


## **트래픽 엔지니어링: 전통적인 라우팅에서는 어려움**

**Q:** 네트워크 운영자가 **u**에서 **z**로 가는 트래픽을 **uvwz**와 **uxyz** (로드 밸런싱)을 통해 분할하려면 어떻게 해야 하나요?

**A:** 이를 할 수 없습니다 (또는 새로운 라우팅 알고리즘이 필요합니다).


## **트래픽 엔지니어링: 전통적인 라우팅에서는 어려움**

**Q:** 만약 **w**가 **w에서 z**로 가는 **파란색**과 **빨간색** 트래픽을 다르게 라우팅하고 싶다면 어떻게 해야 하나요?

**A:** 이를 할 수 없습니다 (목적지 기반 포워딩, LS 및 DV 라우팅으로는 불가능)

**우리는 4장에서** 일반화된 포워딩과 **SDN**을 사용하여 **어떤** 라우팅이든 달성할 수 있다는 것을 배웠습니다.
...
- 빠르고 단순한 범용 스위치들이 **일반화된 데이터 플레인 포워딩(Section 4.4)** 을 하드웨어로 구현함
- 플로우(포워딩) 테이블은 **SDN 컨트롤러의 감독 하에 계산되어 설치됨**
- 테이블 기반 스위치 제어를 위한 **API 제공(OpenFlow 등)**
    - 무엇을 제어할 수 있고, 무엇을 제어할 수 없는지를 정의함
- 스위치가 컨트롤러와 통신하기 위한 **프로토콜 제공(OpenFlow 등)**
...
- **네트워크 상태 정보를 유지함**
- **northbound API**를 통해 위의 네트워크 제어 애플리케이션들과 상호작용함  
    (예: routing, access control, load balancing 등)
- **southbound API**를 통해 아래의 네트워크 스위치들과 상호작용함
- **성능, 확장성, 장애 허용성, 견고성을 위해** 분산 시스템 형태로 구현됨
...
- **제어의 “두뇌” 역할을 함**  
    SDN 컨트롤러가 제공하는 저수준 서비스와 API를 이용하여  
    네트워크 제어 기능을 구현함.
    
- **unbundled(분리됨)**  
    • 네트워크 제어 애플리케이션은 **제3자(3rd party)도 제공할 수 있음**  
    • 라우터 제조사나 SDN 컨트롤러와는 **독립적으로** 개발 가능함
...
- **컨트롤러와 스위치 사이에서 동작함**
- **메시지 교환에는 TCP 사용**
    - 암호화는 **선택적(optional)** 적용 가능

- **OpenFlow 메시지는 세 종류로 구성됨**    
    - controller-to-switch 메시지
    - asynchronous 메시지 (switch → controller)
    - symmetric 메시지 (기타 일반 메시지)

- **OpenFlow API와는 구분됨**    
    - API는 일반화된 포워딩 동작을 지정하는 데 사용됨



## **SDN: 제어 평면 / 데이터 평면 상호작용 예시**

① **S1 스위치가 링크 장애를 감지하면**, OpenFlow 포트 상태(port status) 메시지를 사용하여  
**컨트롤러에 장애 발생을 알림**

② **SDN 컨트롤러는 OpenFlow 메시지를 수신하고**, 링크 상태 정보를 갱신함

③ **Dijkstra 라우팅 알고리즘 애플리케이션은**, 링크 상태가 변경될 때 호출되도록  
**미리 컨트롤러에 등록되어 있으며**, 변경이 발생하면 호출됨

④ **Dijkstra 라우팅 알고리즘은** 컨트롤러 안의 네트워크 그래프 정보와 링크 상태 정보를 읽어  
**새로운 경로를 계산함**

⑤ **링크 상태 라우팅 애플리케이션은 SDN 컨트롤러 내부의  
플로우 테이블 계산(flow-table-computation) 구성 요소와 상호작용하며,  
필요한 새로운 플로우 테이블을 계산함**

⑥ **컨트롤러는 OpenFlow를 사용하여 갱신이 필요한 스위치에  
새로운 플로우 테이블을 설치함**
...
- **자율 시스템(autonomous systems, 즉 “네트워크”)**:  
    수천 개의 상호 작용하는 하드웨어/소프트웨어 구성 요소들로 이루어져 있음.
    
- **감시, 설정, 제어가 필요한 다른 복잡한 시스템들:**
    
    - 제트 비행기, 원자력 발전소 등
        
- **정의(빨간 박스):**  
    “네트워크 관리(network management)는  
    하드웨어, 소프트웨어, 인간 요소를 배치하고 통합하며 조정하여  
    네트워크와 구성 요소 자원을 모니터링, 테스트, 폴링, 설정, 분석, 평가, 제어하는 것을 포함한다.  
    목표는 실시간 운영 성능과 QoS 요구 사항을 합리적인 비용으로 충족하는 것이다.”
    

# 🔵 **간단 요약**

네트워크 관리는 복잡한 네트워크 시스템을 **감시·구성·제어**하는 모든 활동이며,  
실시간 성능과 QoS를 만족시키도록 하드웨어·소프트웨어·사람을 통합적으로 관리하는 과정이다.
...
**Managing server:**  
애플리케이션이며 일반적으로 네트워크 관리자(사람)가 관여하여 운영함.

**Network management protocol:**  
관리 서버가 장비를 조회(query), 구성(configure), 관리(manage)하는 데 사용됨.  
또한 장비가 데이터나 이벤트를 관리 서버에 알릴 때도 사용됨.

(중앙 그림: 관리 서버가 여러 Managed device 안의 agent와 통신하여 data를 교환하는 모습)

**Managed device:**  
관리 가능하며 설정 가능한 하드웨어·소프트웨어 구성 요소를 가진 장비.

**Data:**  
장비의 “상태(state)”,  
구성 데이터(configuration data),  
운영 데이터(operational data),  
장비 통계(device statistics)를 의미함.

# 🔵 **간단 요약**

네트워크 관리는 **관리 서버**, **관리 프로토콜**, **관리되는 장비**, **에이전트**, **장비 데이터**로 구성되며,  
관리 서버가 프로토콜을 이용해 각 장비와 통신하여 설정·상태 모니터링·제어를 수행하는 구조이다.
...
• 운영자가 개별 장비에 직접 명령(타이핑, 스크립트)을 내림 (예: SSH 사용)

**SNMP/MIB**  
• 운영자가 간단한 네트워크 관리 프로토콜(SNMP)을 사용하여  
장비의 데이터(MIB)를 조회하거나 설정함

**NETCONF/YANG**  
• 더 추상적이며, 네트워크 전반을 포괄하는 전체적(holistic) 관리 방식  
• 여러 장비를 동시에 구성하는 관리에 중점을 둠  
• **YANG:** 데이터 모델링 언어  
• **NETCONF:** 원격 장비로부터/에게/간에 YANG 호환 데이터 및 명령을 전달하는 프로토콜

오른쪽 그림: 관리 서버가 여러 관리 대상 장비의 에이전트와 통신하며 데이터를 주고받는 모습.

# 🔵 **간단 요약**

네트워크 관리는 다음 세 가지 방식으로 수행됨:

- **CLI:** 장비 하나씩 직접 명령 입력
- **SNMP:** 표준 프로토콜로 장비 상태를 조회/설정
- **NETCONF/YANG:** 네트워크 전체를 모델링하여 여러 장비를 통합 관리

→ 점점 더 자동화되고 추상화된 방식으로 발전하는 구조이다.



## **YANG**

- NETCONF 네트워크 관리 데이터의 구조, 문법, 의미를 정의하기 위해 사용되는 **데이터 모델링 언어**
    - SMI처럼 기본 제공 데이터 타입을 포함함
- 장비의 기능을 설명하는 XML 문서는 YANG 기술서(description)로부터 생성될 수 있음    
- 유효한 NETCONF 설정이 만족해야 하는 **데이터 간 제약 조건**을 표현할 수 있음
    - NETCONF 설정이 올바름과 일관성 제약을 충족하도록 보장함


오른쪽 그림:  
NETCONF RPC 메시지 안에 YANG으로 생성된 XML이 포함되어 네트워크 장비의 agent로 전달됨.

# 🔵 **간단 요약**

YANG은 **NETCONF에서 사용할 장비 설정 데이터를 정의하는 모델링 언어**이다.  
→ YANG으로 구조를 정의 → NETCONF가 XML 형태로 장비에 전달 → 장비가 일관된 설정을 적용함.
