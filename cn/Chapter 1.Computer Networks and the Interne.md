...
- **실제 인터넷 지연과 손실**이 어떻게 보이는지 확인할 수 있습니다.
- **`traceroute` 프로그램**은 출발지에서 목적지까지의 경로 상 각 **라우터(router)** 에서의 지연 시간을 측정합니다.

## 동작 방식

1. 송신자가 **라우터 i** 까지 도달할 수 있는 **패킷 3개**를 전송합니다. (TTL 값을 i로 설정)    
2. **라우터 i** 는 해당 패킷을 송신자에게 되돌려 보냅니다.
3. 송신자는 **전송과 응답 간의 시간 간격**을 측정하여 **해당 홉의 지연 시간(RTT)** 을 구합니다.
## 요약

- 각 홉마다 3개의 프로브 패킷을 보내 RTT를 측정합니다.    
- 출력값에는 각 라우터까지 걸린 시간(세 번 측정값)이 표시됩니다.
- 이를 통해 **경로**와 **지연 특성**을 파악할 수 있습니다.

# Real Internet Delays and Routes (Traceroute Example)
## 실행 명령
traceroute: gaia.cs.umass.edu www.eurecom.fr

## 결과 요약
- 각 홉(hop)마다 **3개의 지연값(ms)** 표시
- `* * *` : 응답 없음(라우터 응답 차단, 손실)

## 경로 해석
1. cs-gw (1~2ms) → 로컬 게이트웨이  
2. border1 (1~2ms) → 캠퍼스 라우터  
3~7. vbns/abilene (5~22ms) → 미국 내 백본망  
3. 62.40.103.253 (약 100ms) → **대서양 횡단 링크**  
9~12. de1, fr1, renater (100~120ms) → 유럽 백본망  
13~15. nice, valbonne (120~133ms) → 프랑스 지역망  
4. 194.214.211.25 (126ms)  
5. * * * → 응답 없음  
6. fantasia.eurecom.fr (128~136ms) → 최종 목적지  

## 특징
- 미국 → 유럽 구간에서 지연 급증 (trans-oceanic link)  
- 일부 홉에서 지연 감소처럼 보이는 현상: 큐잉 변동/경로 차이 때문  
- 최종 RTT 약 **130ms**, 대륙 간 정상적인 지연 값


# Packet Loss
## 정의
- **Queue(버퍼)** 는 용량이 제한적임
- **가득 찬 큐에 도착한 패킷** → **드롭(손실)**
- 손실된 패킷은
  - 이전 노드에서 재전송되거나
  - 송신 측(end system)에서 재전송되거나
  - 아예 재전송되지 않을 수도 있음

## 동작 과정
1. 송신자 A, B가 라우터로 패킷 전송
2. 라우터는 내부 **버퍼(대기 공간)** 에 저장
3. 전송 중인 패킷이 처리되는 동안 새 패킷은 버퍼에서 대기
4. 버퍼가 가득 찬 상태에서 도착한 패킷은 **손실(packet loss)**

## 요약
- 네트워크 혼잡 발생 시 패킷 손실은 불가피
